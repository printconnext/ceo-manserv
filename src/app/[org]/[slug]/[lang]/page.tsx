...
interface PageProps {
    params: Promise<{ org: string; slug: string; lang: string }>;
}

export default async function ProfilePage({ params }: PageProps) {
    const { org, slug, lang } = await params;

    const dbSlug = `${slug}-${lang.toUpperCase()}`;
    ...
    // Now we must separate the base slug and map to the target language structure
    const baseSlugRaw = profile.slug.replace(/-(TH|EN|CH|JP|LO|HI|FR|IT|ES|DE|RU|FA|PT|BR|VI|MY|PH|ID)$/i, "");

    const availableLanguages = [
        { code: translation.lang.toUpperCase(), langCode: translation.lang, slug: baseSlugRaw, isCurrent: true },
        ...siblings.map(s => {
            const siblingLang = s.translations[0]?.lang || "";
            const siblingBaseSlug = s.slug.replace(/-(TH|EN|CH|JP|LO|HI|FR|IT|ES|DE|RU|FA|PT|BR|VI|MY|PH|ID)$/i, "");
            return {
                code: siblingLang.toUpperCase() || "??",
                langCode: siblingLang,
                slug: siblingBaseSlug,
                isCurrent: false
            };
        })
    ].filter(l => l.langCode !== "");
